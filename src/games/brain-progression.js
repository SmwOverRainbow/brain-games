...
const getProgression = () => {
  ...
  const lengthProgr = _.random(9, 10);
  const firstNumProgr = _.random(0, 10);
  let progression = '';
...
const randomQuest = () => {
  const positionOfMissingNum = _.random(0, 8);
...
const correctAnswer = (string) => {
  ...
  if ((indexOfMissingNum === members.length - 1) || (indexOfMissingNum === members.length - 2)) {
    ...
    missingNum = searchStep + members[indexOfMissingNum - 1];
  } else {
